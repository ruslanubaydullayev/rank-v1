...
by Vercel to the Hetzner backend, so the browser only ever talks to your
`*.vercel.app` domain and login cookies are first-party (no custom domain
needed). The Hetzner box runs the full Nitro API, the render pipeline
(`ffmpeg` + `yt-dlp`), and Postgres in Docker, with free HTTPS via `sslip.io`.

```
Browser (your-project.vercel.app)
  |-- pages ------------------> Vercel (static)
  |-- /api/* -----------------> Vercel rewrite --> https://<ip>.sslip.io (Hetzner)
...
## Part A - Hetzner backend

1. Create a Hetzner VPS (Ubuntu/Debian). Note its public IP, e.g. `203.0.113.5`.
...
5. Open firewall ports 80 and 443 (Hetzner Cloud Firewall in the console, or on
   the box itself: `ufw allow 80,443/tcp`).
...
4. Edit [vercel.json](vercel.json) and replace `REPLACE_WITH_BACKEND_HOST` with your
...
1. Back on Hetzner, set the real Vercel URL and restart:
...
Copy its signing secret into Hetzner `.env` as `STRIPE_WEBHOOK_SECRET`, then
`docker compose up -d`.
...
IP (Hetzner) is gated harder than home connections.
...
- **Login redirect fails / redirect_uri_mismatch**: `NUXT_PUBLIC_SITE_URL` on
  Hetzner must exactly match the Vercel URL, and that URL + `/api/auth/google`
